...
@threestudio.register("sana-guidance")
class SANAGuidance(BaseObject):
    @dataclass
    class Config(BaseObject.Config):
        pretrained_model_name_or_path: str = "Efficient-Large-Model/Sana_600M_512px_diffusers"
    ...
    def configure(self) -> None:
        threestudio.info(f"Loading SANA ...")

        self.weights_dtype = (
            torch.float16 if self.cfg.half_precision_weights else torch.float32
        )
        self.torch_variant = "fp16" if self.cfg.half_precision_weights else "fp32"
        self.pipe = SanaPipeline.from_pretrained(
            self.cfg.pretrained_model_name_or_path,
            variant=self.torch_variant,
        ...
        # self.scheduler = DPMSolverMultistepScheduler_ext.from_pretrained(
        #     self.cfg.pretrained_model_name_or_path,
        #     subfolder="scheduler",
        # )
        ...
        self.scheduler.set_timesteps(40, device=self.device)
        ...
        self.scheduler.sigmas = self.sigmas

        self.sampling_index = 0 # sdi mod

        # self.inverse_scheduler = FlowMatchEulerDiscreteInverseScheduler.from_pretrained(
        #     self.cfg.pretrained_model_name_or_path,
        #     subfolder="scheduler",
        # )
        ...
        threestudio.info(f"Loaded SANA!")

    # @torch.autocast(device_type="cuda", enabled=False)
    # def set_min_max_steps(self, timesteps):
    #     self.min_step = timesteps[-1]
    #     self.max_step = timesteps[0]
    ...
    ) -> Float[Tensor, "B 32 16 16"]:
        input_dtype = imgs.dtype
        imgs = imgs * 2.0 - 1.0
        latents = self.vae.encode(imgs.to(self.vae.dtype)).latent * self.vae.config.scaling_factor
        return latents.to(input_dtype)

    @torch.autocast(device_type="cuda", enabled=False)
    def decode_latents(
        self,
        latents: Float[Tensor, "B 32 H W"],
        output_type="np"
    ...
    def predict_flow(
        ...
        if prompt_utils.use_perp_neg:
            ...
            if text_embeddings is None:
                text_embeddings, text_attention_masks = prompt_utils.get_text_embeddings(
                    elevation,
                    azimuth,
                    camera_distances,
    ...
    def get_x0(self, original_samples, flow_pred, t):
        # step_results = self.scheduler.step(
        #     flow_pred, t[0].to(torch.float32), original_samples, return_dict=True
        # )
        # sigma_idx = self.step_index
        index = torch.nonzero(self.timesteps == t[0], as_tuple=True)[0]
        sigma = self.sigmas[index][0]
        ...
        x0_pred = original_samples - sigma * flow_pred
        noise_pred = original_samples + (1 - sigma) * flow_pred
    ...
    def add_noise(self, sample, noise, t):
        ...
        # next_sigma = sigma_next
        # dt = sigma_next - sigma
        noised_sample = sigma * noise + (1.0 - sigma) * sample
    ...
    def compute_grad_sdi(
        ...
        flow_pred, neg_guidance_weights, text_embeddings = self.predict_flow(
            latents_noisy,
            t,
            prompt_utils,
            elevation,
        ...
        latents_denoised, noise_pred = self.get_x0(
            latents_noisy, flow_pred, t
        ) # (latents_noisy - sigma * noise_pred) / alpha
    ...
    def __call__(
        ...
        **kwargs,
        ...
        latents: Float[Tensor, "B 32 16 16"]
        ...
        #     device=self.device,
        # )
        t = self.timesteps[self.sampling_index].repeat(batch_size)
        ...
        if test_info:
            guidance_out["target"] = self.decode_latents(target)
            guidance_out["target_latent"] = target
            guidance_out["noisy_img"] = self.decode_latents(noisy_img)
            guidance_out["noise_img"] = self.decode_latents(noise)
    ...
    def update_step(self, ):
        ...
        if self.cfg.t_anneal:
            ...
            # )  # progress percentage
            # step_float = self.cfg.min_step_percent + percentage * (
            #     self.cfg.max_step_percent - self.cfg.min_step_percent
            # )
            self.sampling_index = self.sampling_index + 1
